...
import RecentMCQCard from "./RecentMCQCard";
...
const RecentMCQ = () => {
  ...
  const recentBlogs = data?.data
    ? [...data.data]
        .filter((mcq: IMCQ) => {
          if (mcq.createdAt) {
            const createdAt = new Date(mcq.createdAt);
  ...
  return (
    <div className="shadow-md border-[0.5px] border-gray-800 rounded-md">
      <div className="bg-gray-900 rounded-t-md text-gray-300 p-2 shadow-sm">
        <h1 className="text-lg hind-siliguri-semibold ml-2">Recent MCQ</h1>
      </div>
      <div className="p-4">
        <div>
          {blogsToDisplay.length > 0
            ? blogsToDisplay.map((mcq: IMCQ, index) => (
                <RecentMCQCard
                  mcq={mcq}
                  key={mcq._id}
